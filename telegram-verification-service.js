...
app.post('/verify-telegram-channel', async (req, res) => {
  try {
    ...
    // Clean channel ID - ensure @ prefix for public channels
    // Telegram API requires @ for public channels (e.g., @neftittest)
    // Numeric IDs (e.g., -1001234567890) don't need @
    let cleanChannelId = channelId.trim();

    // If it's a numeric ID (starts with - or is all digits), use as-is
    // Otherwise, ensure it has @ prefix for public channels
    ...
    console.log('📡 [Telegram] Cleaned channel ID:', cleanChannelId);
